...
export default function LandInspectorLogin() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch("/api/auth/land-inspector-login", {
        method: "POST",
    ...
    <div className="min-h-screen bg-slate-100 flex items-center justify-center p-4">
      <div className="max-w-md w-full bg-white p-8 rounded-lg shadow-lg border">
        <div className="text-center mb-6">
          <h1 className="text-2xl font-bold text-slate-900">Land Inspector Login</h1>
          <p className="text-slate-600 mt-2">Access the land mapping portal</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          >
            {isLoading ? "Signing In..." : "Sign In to Land Portal"}
          </button>
        </form>
      </div>
    </div>
