...
async function sendStageNotifications(
  ...
  const notifiedStakeholders = Array.isArray(masterTx.stakeholdersNotified) ? masterTx.stakeholdersNotified : [];
  ...
  */
  ...
  app.post("/api/buyer/request-payment-to-exporter", async (req, res) => {
    try {
      ...
      // 🔗 DYNAMIC UPDATE: Find correct inspection booking ID from database
      const inspectionBooking = await db
        .select()
        .from(portInspectionBookings)
        .where(eq(portInspectionBookings.transactionId, custodyId))
        .limit(1);
  ...
  app.post("/api/exporter/confirm-payment/:bookingId", async (req, res) => {
    try {
      ...
      if (inspectionCompletionStatus[bookingId]) {
        inspectionCompletionStatus[bookingId].paymentConfirmed = true;
        inspectionCompletionStatus[bookingId].exporterPaymentStatus = 'PAYMENT_CONFIRMED';
        inspectionCompletionStatus[bookingId].paymentConfirmedAt = confirmedAt;
        inspectionCompletionStatus[bookingId].completedWorkflow = true;
      }
  ...
  app.post("/api/buyer/validate-payment", async (req, res) => {
    try {
      ...
      if (bookingId && inspectionCompletionStatus[bookingId]) {
        inspectionCompletionStatus[bookingId].paymentValidated = true;
        inspectionCompletionStatus[bookingId].paymentValidatedAt = validatedAt;
        inspectionCompletionStatus[bookingId].exporterPaymentStatus = 'TRANSACTION_COMPLETED';
        inspectionCompletionStatus[bookingId].workflowCompleted = true;
      }
  ...
  app.get("/api/exporter/payment-request-status/:bookingId", async (req, res) => {
    try {
      ...
      // 🔗 DYNAMIC STATUS CHECK: Find correct inspection booking ID from database
      const inspectionBooking = await db
        .select()
        .from(portInspectionBookings)
  ...
  // ==================== DG COMPREHENSIVE MONITORING APIS ====================
  
  // Mobile Inspector Monitoring
  app.get('/api/dg/monitoring/mobile-inspectors', async (req, res) => {
    try {
      ...
      console.error('Error fetching mobile inspector data:', error);
      res.status(500).json({ error: 'Failed to fetch monitoring data' });
    }
  });

  // County-Wide Comprehensive Reports
  app.get('/api/dg/monitoring/county-reports', async (req, res) => {
    try {
      const counties = [
      ...
      const countyData = counties.map(county => ({
        ...
        compliance: (85 + Math.random() * 15).toFixed(1)
      }));

      res.json({ counties: countyData, totalCounties: 15, activeCounties: 15 });
    } catch (error) {
      console.error('Error fetching county reports:', error);
      res.status(500).json({ error: 'Failed to fetch county reports' });
    }
  });

  // Supply Chain Monitoring
  app.get('/api/dg/monitoring/supply-chain', async (req, res) => {
    try {
      const supplyChainData = {
        farmToHarvest: {
          activeBatches: 247,
          autoGeneratedCodes: 89,
          completionRate: 87
        },
        warehouseProcessing: {
          inStorage: 156,
  ...
  app.get('/api/dg/monitoring/portal-activity', async (req, res) => {
    try {
  ...
  app.get('/api/dg/monitoring/compliance', async (req, res) => {
    try {
      const complianceData = {
        eudrCompliance: {
          lacraStandards: 96.8,
          landMappingCompliance: 89.2,
          qualityInspections: 94.1
        },
        auditTrail: [
  ...
  app.get('/api/dg/monitoring/emergency', async (req, res) => {
    try {
      const emergencyData = {
        criticalAlerts: [
          { 
  ...
  app.get('/api/dg/monitoring/analytics', async (req, res) => {
    try {
      const analyticsData = {
        performanceMetrics: [
  ...
  app.get('/api/dg/monitoring/overview', async (req, res) => {
    try {
      const overviewData = {
        activeInspectors: 47,
        countiesOnline: 15,
  ...
  */
  ...
  const masterTransactionRegistry = {
    'TXN-FPO-20250830-817649-923': {
      ...
      // 🛒 BUYER STAGE - IDENTIFIED: BYR-20250827-041
      buyerId: 'BYR-20250827-041', // ✅ BUYER WHO ACCEPTED CLAUDIO'S OFFER
      buyerName: 'Vivaan Gupta', // ✅ Confirmed from logs: accepted FPO-20250830-817649-923
      buyerCompany: 'VIVAAN AGRO LTD', // ✅ Actual company from credentials 
  ...
  app.get("/api/port-inspector/qr-image/:inspectionId", async (req, res) => {
    try {
      ...
      // 🎯 GET REAL QR CODE FOR THIS SPECIFIC INSPECTION - NO DUMMY DATA
      // Step 1: Get the inspection booking and its transaction ID
      const inspectionResult = await db.execute(sql`
        SELECT transaction_id, commodity_type FROM port_inspection_bookings 
        WHERE booking_id = ${inspectionId}
        LIMIT 1
      `);
      ...
      // Step 2: Get the correct QR batch code from warehouse custody
      const warehouseCustodyResult = await db.execute(sql`
        SELECT product_qr_codes FROM warehouse_custody 
        WHERE custody_id = ${inspection.transaction_id}
        LIMIT 1
      `);
      ...
      if (warehouseCustodyResult.rows.length === 0) {
        return res.status(404).json({ 
          success: false, 
          message: "Warehouse custody record not found"
        });
      }
      
      const productQrCodes = warehouseCustodyResult.rows[0].product_qr_codes;
      const actualQrBatchCode = Array.isArray(productQrCodes) && productQrCodes.length > 0 ? 
        productQrCodes[0] : null;
      
      if (!actualQrBatchCode) {
        return res.status(404).json({ 
          success: false, 
          message: "QR batch code not found for this inspection"
        });
      }
      
      // Step 3: Get the real QR batch data from database
      const qrBatchResult = await db.execute(sql`
        SELECT batch_code, qr_code_data, commodity_type, farmer_name, 
               total_bags, total_weight, warehouse_name
        FROM qr_batches 
        WHERE batch_code = ${actualQrBatchCode}
        LIMIT 1
      `);
      ...
      if (qrBatchResult.rows.length === 0) {
        return res.status(404).json({ 
          success: false, 
          message: "QR batch data not found"
        });
      }
      
      const qrBatch = qrBatchResult.rows[0];
      
      // 🎯 RETURN REAL DYNAMIC DATA FOR THIS SPECIFIC INSPECTION
      const qrImageData = {
        ...
        batchCode: qrBatch.batch_code, // Dynamic batch code for this inspection
        productInfo: {
      ...
      console.log(`✅ QR code image fetched for ${qrBatch.batch_code} (inspection: ${inspectionId})`);
...
VERIFY: ${qrCodeData.verificationUrl}`;
  ...
  // Buyer-Exporter payment confirmation workflow removed as requested

  // Buyer payment validation workflow removed as requested
    ...
    try {
      ...
      res.json({
        ...
        data: confirmedRequests.rows.map((row: any) => {
          // Debug removed - QR workflow locked in place
  ...
  // REMOVED OLD DISPATCH CONFIRMATION ENDPOINT - QR WORKFLOW LOCKED
  // Dispatch confirmation now handled via get-dispatch-qr which displays existing custody QR codes
  // This maintains proper traceability: Buyer requests → Warehouse generates QR → Custody stores QR → Dispatch displays same QR
  
  // All dispatch operations now use custody-based QR codes - no new generation during dispatch
  ...
  app.get("/api/buyer/custody-lots/:buyerId", async (req, res) => {
    try {
      ...
      // 🔗 DYNAMIC DATABASE LOOKUP: Get inspection bookings for payment workflow
      const inspectionBookings = await db
        .select()
        .from(portInspectionBookings)
        .where(inArray(portInspectionBookings.transactionId, custodyLots.map(lot => lot.custodyId)));
