...
export function registerAutoEudrRoutes(app: Express) {
  ...
  // Download individual EUDR document as PDF
  app.get('/api/eudr/download-document/:packId/:documentType', async (req, res) => {
    try {
  ...
  // Download all documents as a bundle
  app.get('/api/eudr/download-all/:packId', async (req, res) => {
    try {
      const { packId } = req.params;
      ...
      // Create comprehensive text document as fallback
...
`;
      ...
      console.log(`📄 Document bundle downloaded for pack: ${packId}`);
...
function getDocumentFileName(documentType: string, packId: string): string {
  const fileNames: Record<string, string> = {
    'cover': `EUDR_Cover_Sheet_${packId}.pdf`,
    'certificate': `LACRA_Export_Certificate_${packId}.pdf`,
    'assessment': `EUDR_Compliance_Assessment_${packId}.pdf`,
    'deforestation': `Deforestation_Analysis_Report_${packId}.pdf`,
    'diligence': `Due_Diligence_Statement_${packId}.pdf`,
    'traceability': `Supply_Chain_Traceability_${packId}.pdf`
  };
